...
def function(n, m, f):
  '''Assumes that n = m = 1. The argument f is a Python function that takesas input an n-bit string alpha and returns as output an m-bit string f(alpha). See deutschTest for examples of f. This function returns the (n + m)-qbit gate F that corresponds to f.'''
  if f((1,)) == (1,) and f((0,)) == (0,):
    return cnot
...
def deutsch(f):
  '''Given a two-qbit gate representing a function f : {0, 1} -> {0, 1},outputs ket1 if f is constant and ket0 if f is not constant.'''
...
def tensor(a, b):
  '''Assumes that a and b are both gates or a and b are both states. Let a be n-qbit and b be m-qbit, where n, m >= 1. Returns the tensor product of a and b, which is (n + m)-qbit.'''
  if np.shape(a) == (1,): # if a is-a state
    tp = np.array([])
    for alpha in a:
      for beta in b:
        np.append(tp, a * b)
...
def first(state):
  '''Assumes n >= 1. Given an n-qbit state, measures the first qbit. Returnsa pair (a tuple or list of two elements) consisting of a classical one-qbit state (either ket0 or ket1) and an (n - 1)-qbit state.'''
  sumx = 0
...
def firstTest345(n, m):
  '''Assumes n >= 1. Uses one more qbit than that, so that the total numberof qbits is n + 1. The parameter m is how many tests to run. Should returna number close to 0.64 --- at least for large m.'''
  psi0 = 3 / 5
  beta = uniform(n)
  psi1 = 4 / 5
  gamma = uniform(n)
